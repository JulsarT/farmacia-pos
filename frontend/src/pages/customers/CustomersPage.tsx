...
import { Plus, Search, Edit3, Trash2, Users, Phone, Mail, Loader2, X, Save } from 'lucide-react'
...
export default function CustomersPage() {
  ...
  const handleEdit = (c: any) => { setEditingCustomer(c); setModalOpen(true) }
  const handleNew = () => { setEditingCustomer(null); setModalOpen(true) }
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Gestión de clientes y su historial de compras</p>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-800)' }}>
                {['Nombre / Razón Social', 'NIT / CI', 'Contacto', 'Total Compras', 'Acciones'].map((h) => (
            ...
            <tbody>
              {customers.map((c: any) => (
                <tr key={c.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '12px 16px', fontSize: 13, fontFamily: 'monospace' }}>{c.nit || '-'}</td>
                  <td style={{ padding: '12px 16px', fontSize: 13 }}>
                    {c.phone && <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: c.email ? 4 : 0 }}><Phone size={12} color="var(--text-400)"/>{c.phone}</div>}
                    {c.email && <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Mail size={12} color="var(--text-400)"/>{c.email}</div>}
                    {!c.phone && !c.email && <span style={{color: 'var(--text-400)'}}>-</span>}
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <div className="badge badge-primary">{c._count?.sales || 0} compras</div>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button className="btn btn-ghost" style={{ padding: '6px 10px' }} onClick={() => handleEdit(c)}>
                        <Edit3 size={14} />
                      </button>
                      <button className="btn btn-ghost" style={{ padding: '6px 10px', color: 'var(--danger)' }}
                        onClick={() => { if (confirm(`¿Eliminar cliente ${c.name}?`)) deleteMutation.mutate(c.id) }}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
...
function CustomerModal({ open, onClose, customer, onSaved }: { open: boolean, onClose: () => void, customer: any, onSaved: () => void }) {
  const [form, setForm] = useState({ name: '', nit: '', phone: '', email: '', address: '' })
  
  useEffect(() => {
    if (customer) setForm({ name: customer.name || '', nit: customer.nit || '', phone: customer.phone || '', email: customer.email || '', address: customer.address || '' })
    else setForm({ name: '', nit: '', phone: '', email: '', address: '' })
  }, [customer, open])
      ...
      <div className="card" style={{ width: 500, animation: 'slideUp 0.2s ease' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <h3 style={{ margin: 0, fontSize: 18 }}>{customer ? 'Editar Cliente' : 'Nuevo Cliente'}</h3>
          </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form) }} style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div><label style={{ fontSize: 12, color: 'var(--text-300)', marginBottom: 4, display: 'block' }}>Nombre Completo *</label><input className="input" required value={form.name} onChange={e => setForm({...form, name: e.target.value})} /></div>
            <div><label style={{ fontSize: 12, color: 'var(--text-300)', marginBottom: 4, display: 'block' }}>NIT / CI</label><input className="input" value={form.nit} onChange={e => setForm({...form, nit: e.target.value})} /></div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
