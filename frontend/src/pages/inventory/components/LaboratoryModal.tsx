...
import { X, Save, Loader2, FlaskConical } from 'lucide-react'
...
export default function LaboratoryModal({ open, onClose, laboratory, onSaved }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: (data: { name: string }) =>
      laboratory
        ? api.put(`/products/laboratories/${laboratory.id}`, data)
        : api.post('/products/laboratories', data),
    onSuccess: onSaved,
      ...
      <div className="card" style={{ width: '100%', maxWidth: 400 }}>
        <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <h3 style={{ margin: 0, fontSize: 16 }}>{laboratory ? 'Editar Laboratorio' : 'Nuevo Laboratorio'}</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding: 20 }}>
          ...
          <label style={{ display: 'block', fontSize: 12, fontWeight: 500, marginBottom: 6, color: 'var(--text-300)' }}>Nombre del Laboratorio *</label>
          <input className="input" placeholder="Ej: Bayer, Roche, Farmabolivia..." value={name} onChange={(e) => setName(e.target.value)} autoFocus />
